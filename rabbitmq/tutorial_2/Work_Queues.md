...
在第一门课程，我们写了一段程序，来对指定的`queue`发送和接收信息。而在这门课程中，我们将创建一个`Work Queue`对多个`workers`进行分发`time-consuming`任务。

`Work Queues`(即`Task Queues`)背后的中心思想就是，避免直接运行一个不得不等待它完成的`resource-intensive`任务。相反，我们计划将任务延后执行。我们把任务封装成`message`，然后发送到`queue`。运行在后台的`worker`将会弹出这些任务，且最终执行这些工作。当你运行多个`worker`时，任务将会在它们之间共享。
